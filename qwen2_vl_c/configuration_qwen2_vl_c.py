...
''' pruning_lists and rank_lists are dictionaries containing lists of pruning ratios and ranks for each layer.
    ...
    - "sa_out": for self-attention output weights
    - "mlp_gate": for MLP gate weights # ONLY FOR TEXT CONFIG
    - "mlp_down": for MLP down weights
...
# TODO: Pruning of all the layers connecting to skip connections should have the same pruning ratio.
        ...
        **kwargs,
        ...
        if pruning_ratio_lists is not None:
            ...
            if pruning_ratio_lists.get("all") is not None:
                ...
                self.pruning_ratio_lists["mlp_down"] = pruning_ratio_lists["all"]
            ...
            if pruning_ratio_lists.get("sa") is not None:
                ...
                self.pruning_ratio_lists["sa_out"] = pruning_ratio_lists["sa"]
            if pruning_ratio_lists.get("mlp") is not None:
                self.pruning_ratio_lists["mlp_down"] = pruning_ratio_lists["mlp"]
        ...
        if lrd_rank_lists is not None:
            ...
            if lrd_rank_lists.get("all") is not None:
                self.lrd_rank_list["sa_qkv"] = lrd_rank_lists["all"]
                self.lrd_rank_list["sa_out"] = lrd_rank_lists["all"]
                self.lrd_rank_list["mlp_down"] = lrd_rank_lists["all"]
                self.lrd_rank_list["mlp_up"] = lrd_rank_lists["all"]
            if lrd_rank_lists.get("sa") is not None:
                self.lrd_rank_list["sa_qkv"] = lrd_rank_lists["sa"]
                self.lrd_rank_list["sa_out"] = lrd_rank_lists["sa"]
            if lrd_rank_lists.get("mlp") is not None:
                self.lrd_rank_list["mlp_down"] = lrd_rank_lists["mlp"]
                self.lrd_rank_list["mlp_up"] = lrd_rank_lists["mlp"]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pruning_ratio_lists is not None:
            # Use general-type keys to substitute the keys for each layer type
            if pruning_ratio_lists.get("all") is not None:
                self.pruning_ratio_lists["sa_q"] = pruning_ratio_lists["all"]
                self.pruning_ratio_lists["sa_k"] = pruning_ratio_lists["all"]
                self.pruning_ratio_lists["sa_v"] = pruning_ratio_lists["all"]
                self.pruning_ratio_lists["sa_out"] = pruning_ratio_lists["all"]
        ...
        if lrd_rank_lists is not None:
            # Use general-type keys to substitute the keys for each layer type
            if lrd_rank_lists.get("all") is not None:
                self.rank_list["sa_q"] = lrd_rank_lists["all"]
                self.rank_list["sa_k"] = lrd_rank_lists["all"]
                self.rank_list["sa_v"] = lrd_rank_lists["all"]
                self.rank_list["sa_out"] = lrd_rank_lists["all"]
                self.rank_list["mlp_gate"] = lrd_rank_lists["all"]
                self.rank_list["mlp_down"] = lrd_rank_lists["all"]
                self.rank_list["mlp_up"] = lrd_rank_lists["all"]
            if lrd_rank_lists.get("sa") is not None:
                self.rank_list["sa_q"] = lrd_rank_lists["sa"]
                self.rank_list["sa_k"] = lrd_rank_lists["sa"]
                self.rank_list["sa_v"] = lrd_rank_lists["sa"]
                self.rank_list["sa_out"] = lrd_rank_lists["sa"]
            if lrd_rank_lists.get("mlp") is not None:
                self.rank_list["mlp_gate"] = lrd_rank_lists["mlp"]
                self.rank_list["mlp_down"] = lrd_rank_lists["mlp"]
                self.rank_list["mlp_up"] = lrd_rank_lists["mlp"]

            # Substitute the keys for each layer type if they are defined
            self.rank_list.update(lrd_rank_lists)       
