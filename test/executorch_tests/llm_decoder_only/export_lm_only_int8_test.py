...
from torch.export import Dim, draft_export
...
def quantize_module(module, example_inputs):
    module.eval()
    exported = draft_export(module, example_inputs)
    prepared = prepare_pt2e(exported.module(), quantizer)
    # No calibration here — run calibration externally on `prepared` before calling this,
    # or load pre-calibrated observer state into `prepared` before convert_pt2e.
    converted = convert_pt2e(prepared)
    quantized = draft_export(converted, example_inputs)
...
if EXPORT_TO_EXECUTORCH:
    ...
    if EXPORT_EMBEDDING:
        ...
        prepared_embedding, quantized = quantize_module(wrapped_embedding, example_inputs_embedding)
    ...
    if EXPORT_LM_DECODER:
        ...
        prepared_decoder, quantized = quantize_module(wrapped_decoder, example_inputs_decoder)
        edge = to_edge_transform_and_lower(
            quantized,
            compile_config=EdgeCompileConfig(_check_ir_validity=CHECK_IR_VALIDITY),
            partitioner=[PARTITIONER],
    ...
    if EXPORT_FINAL_LAYER:
        ...
        prepared_final_layer, quantized = quantize_module(wrapped_final_layer, example_inputs_final_layer)
        edge = to_edge_transform_and_lower(
            quantized,
            compile_config=EdgeCompileConfig(_check_ir_validity=CHECK_IR_VALIDITY),
            partitioner=[PARTITIONER],
