...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Global quant precision (use 'full' for inference-only export)",
    )
    parser.add_argument(
        "--soc-model",
        type=str,
        default="SM8650",
        help="Qualcomm target chipset, e.g. SM8650, SM8550, SM8750",
    )
    parser.add_argument(
        "--max-sequence-length",
        type=int,
    ...
    parser.add_argument(
        "--online-prepare",
        action="store_true",
        help="Enable online prepare in Qualcomm compile spec",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
...
def main():
    ...
    if args.static_seq_len_1:
        ...
        # Static export still uses fixed example shapes; this path just uses seq_len=3.
    ...
    output_path = os.path.join(
        args.out_dir,
        f"export_{args.mode}_{args.backend}_{args.precision}_{args.soc_model}"
