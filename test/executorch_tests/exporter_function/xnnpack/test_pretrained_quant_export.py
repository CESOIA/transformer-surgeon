...
class TestQwen2XNNPACKExport(unittest.TestCase):
    ...
    Subclasses override ``CACHE_IMPL`` to cover the io_* cache contracts; the
    base class exercises the default in-place "mutable" cache.
    """
    ...
    def setUpClass(cls):
        ...
        # io modes: the KV cache is explicit graph I/O; hold zero caches to feed in.
        ...
        try:
            ...
            # io modes flatten the KV-cache lists into the runtime argument list;
            # the exported program returns [logits, *new_key_caches, *new_value_caches].
            if cls.is_io:
                exec_inputs = [
                    cls.stored_input_ids, cls.stored_pos_ids,
                    *cls.stored_key_caches, *cls.stored_value_caches,
...
class TestQwen2XNNPACKExportIOScatter(TestQwen2XNNPACKExport):
    """Same end-to-end pipeline with the functional index_put (io_scatter) cache."""
    CACHE_IMPL = "io_scatter"


class TestQwen2XNNPACKExportIOConcat(TestQwen2XNNPACKExport):
