...
# Cache implementation and per-layer cache geometry are read from each engine's
# <stem>.manifest.json (written by the exporter), so mutable and io_* engines
# run the same way.
ENGINE_FILES=(
    ...
    "$OUT_DIR/qwen2_tensorrt_full_seed0_io_scatter.pt2"
    "$OUT_DIR/qwen2_tensorrt_full_seed0_io_concat.pt2"
)
